...
func run(args []string) {
	...
	fs.DurationVar(&cfg.FlushDelay, "flush-delay", cfg.FlushDelay, "aggregation flush delay; 0 disables waiting")
	...
	fs.DurationVar(&cfg.ActivePollDuration, "active-poll-duration", cfg.ActivePollDuration, "receive poll active period after local activity")
	fs.Uint64Var(&cfg.WindowChunks, "window-chunks", cfg.WindowChunks, "window chunks")
	fs.IntVar(&cfg.CloseCheckAfterMisses, "close-check-after-misses", cfg.CloseCheckAfterMisses, "data misses before close HEAD check")
