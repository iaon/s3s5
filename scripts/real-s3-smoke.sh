...
export S3S5_PROVIDER=${S3S5_PROVIDER:-aws}
if [ "$S3S5_PROVIDER" = "yandex" ] || [ "$S3S5_PROVIDER" = "yc" ]; then
  export S3S5_REGION=${S3S5_REGION:-ru-central1}
else
  : "${AWS_REGION:?set AWS_REGION or use S3S5_PROVIDER=yandex}"
fi
...
./bin/s3s5-server --provider "$S3S5_PROVIDER" --bucket "\$S3S5_BUCKET" --prefix "$S3S5_PREFIX" --region "${S3S5_REGION:-$AWS_REGION}" --allow-target example.com:443
./bin/s3s5-client --provider "$S3S5_PROVIDER" --listen 127.0.0.1:1080 --bucket "\$S3S5_BUCKET" --prefix "$S3S5_PREFIX" --region "${S3S5_REGION:-$AWS_REGION}"
